...
from external.mmdetection3d.tools.data_converter.kitti_data_utils import WaymoInfoGatherer, get_kitti_image_info
from external.mmdetection3d.tools.data_converter.nuscenes_converter import post_process_coords
...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    for info in mmcv.track_iter_progress(kitti_infos):
        coco_infos = get_2d_boxes(root_path, info, occluded=[0, 1, 2, 3], mono3d=mono3d)
        (height, width,
...
def get_2d_boxes(data_path, info, occluded, mono3d=True):
    ...
    # Get image shape
    img_h, img_w = info['image']['image_shape']
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        loc_3d = np.copy(loc)
        loc_3d[0, 0] += offset
        gt_bbox_3d = np.concatenate([loc, dim, rot], axis=1).astype(np.float32)  # cam0
        corner_coords, all_corner_coords, _ = project_corners(gt_bbox_3d, P2)

        # Keep only corners that fall within the image.
        final_coords = post_process_coords(corner_coords, (img_w, img_h))
        ...
        if mono3d and (repro_rec is not None):
            repro_rec['bbox_cam3d'] = np.concatenate(
                [loc_3d, dim, rot],
                axis=1).astype(np.float32).squeeze().tolist()  # cam2
            repro_rec['bbox_cam3d_cam0'] = np.concatenate(
                [loc, dim, rot],
                axis=1).astype(np.float32).squeeze().tolist()  # cam0
            ...
            mark_kpts(projected_pts, img_w, img_h)
            ...
            bboxes_2d.append([min_x, min_y, max_x, max_y])
...
def project_corners(bbox_3d, P2):
    # Filter out the corners that are not in front of the calibrated
    # sensor.
    corners_3d = box_np_ops.center_to_corner_box3d(
        bbox_3d[:, :3],
        bbox_3d[:, 3:6],
        bbox_3d[:, 6], [0.5, 0.5, 0.5],
        axis=1)
    corners_3d = corners_3d[0].T  # (1, 8, 3) -> (3, 8)
    all_corners_3d = corners_3d.copy()
    valid_corners_mask = np.zeros((8, 1))
    corners_depth = all_corners_3d[2].copy()

    in_front = np.argwhere(corners_3d[2, :] > 0).flatten()
    corners_3d = corners_3d[:, in_front]

    # mark valid corners (depth > 0) as 1 (labeled but not visible, similar as COCO)
    ...
    corner_coords = view_points(corners_3d, camera_intrinsic,
                                True).T[:, :2].tolist()
    all_corner_coords = view_points(all_corners_3d, camera_intrinsic, True).T[:, :2]  # (8, 2)
    all_corner_coords = np.hstack([all_corner_coords, valid_corners_mask])  # (8, 3)

    return corner_coords, all_corner_coords, corners_depth


def mark_kpts(kpts, img_w, img_h):
    # further mark points inside the image as visible, the rest is invisible but labelled
    for kpt_idx in range(len(kpts)):
        kptx, kpty, vis = kpts[kpt_idx]
        is_kpt_in_image = (0 <= kptx <= img_w) and (0 <= kpty <= img_h)
        if is_kpt_in_image:
            kpts[kpt_idx, 2] = 2